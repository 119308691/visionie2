...
public class AdminLoginServlet extends HttpServlet {
    ...
    protected void doPost(HttpServletRequest request, HttpServletResponse response)
        ...
        if (StringUtils.isStringEmpty(PPSN) || StringUtils.isStringEmpty(password)) {
            ...
            patient Patient = pServ.loginPatient(PPSN, password);
            if (Patient == null) {
                RequestDispatcher rd = request.getRequestDispatcher("/index.jsp");
                rd.forward(request, response);
            } else {
                request.getSession(true).setAttribute(IConstants.SESSION_KEY_USER, Patient);
                if (Patient.getUserType().equals(IConstants.USER_TYPE_PATIENT)) {
